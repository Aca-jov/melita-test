...
app.controller('AppCtrl', ['$scope','$mdSidenav', '$http', function($scope, $mdSidenav, $http){
  $http({
      ...
      $scope.offers.forEach(function(offer){
        var parts = offer.contractEndDate.split('-');
        var mydate = new Date(parts[0], parts[1]-1, parts[2]);
        offer.expired = mydate < new Date() ? true : false;
      })    
  });
  ...
  $scope.showDetails = function(offerId) {
      ...
      $http({
          method: 'GET',
          url: 'https://selfcare-service.demo.melita.com/interview/api/offers/' + offerId + '/subscriptions'
      }).then(function(result){
          $scope.selectedOffer = $scope.offers.find(x => x.id === offerId);
          $scope.subscriptions = result.data.subscriptions;
          $scope.subscriptions.forEach(function(subscription) {
            switch (subscription.type) {
              case 'MOBILE':
                subscription.icon = 'phone_android';
                break;
              case 'INTERNET':
                subscription.icon = 'network_wifi';
                break;
              case 'TELEVISION':
                subscription.icon = 'tv';
                break;
              case 'TELEPHONY':
                subscription.icon = 'phone';
                break;
            }
          })
      });
